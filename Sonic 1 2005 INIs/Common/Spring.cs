...
namespace S12005ObjectDefinitions.Common
{
    class Spring : ObjectDefinition
    {
        ...
        {
            byte[] artfile1 = ObjectHelper.OpenArtFile("../artnem/springh.bin", CompressionType.Nemesis);
            byte[] artfile2 = ObjectHelper.OpenArtFile("../artnem/springv.bin", CompressionType.Nemesis);
            string map = "../_maps/obj41.asm";
            img = ObjectHelper.MapASMToBmp(artfile1, map, 0, 0);
            imgs.Add(img); // 0x00
            imgs.Add(ObjectHelper.MapASMToBmp(artfile1, map, 0, 1)); // 0x02
            imgs.Add(ObjectHelper.MapASMToBmp(artfile2, map, 3, 0)); // 0x10
            imgs.Add(ObjectHelper.MapASMToBmp(artfile2, map, 3, 1)); // 0x12
        ...
        {
            get { return new ReadOnlyCollection<byte>(new byte[] { 0x00, 0x02, 0x10, 0x12, 0x20, 0x22, 0x30, 0x32 }); }
        }
