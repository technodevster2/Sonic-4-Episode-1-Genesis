...
loc_71F12:
		...
		beq.s	loc_71F3E		; If negative (i.e., it was $80 or lower), branch
		cmpi.b	#$DF,d0		; Is it a special command?		; Clownacy | Part of S2's SFX-only priority system
		bhs.s	loc_musicorflag		; If so, branch				; Clownacy | Part of S2's SFX-only priority system		
		subi.b	#$81,d0
		bcs.s	loc_musicorflag
		cmpi.b	#$80,9(a6)
		beq.s	loc_71F2C
		move.b	d1,$A(a6)
		bra.s	loc_71F3E
...
loc_musicorflag:
		cmpi.b	#$80,9(a6)		; Is v_playsnd0 a $00 (empty)?		; Clownacy | Part of S2's SFX-only priority system
		beq.s	loc_skippriority		; If yes, branch			; Clownacy | Part of S2's SFX-only priority system
		move.b	d1,$A(a6)	; Put sound into v_playsnd1		; Clownacy | Part of S2's SFX-only priority system
		bra.s	loc_71F3E							; Clownacy | Part of S2's SFX-only priority system
; ===========================================================================
...
cfSilenceStopTrack:	; Clownacy | Replaces the old cfSilenceStopTrack, saving a branch
	pea	loc_72D58(pc)	; Clownacy | Save 2 cycles
FMSilenceChannel:
	moveq	#3,d4
	moveq	#$40,d3
	moveq	#$7F,d1
...
sub_7260C:				; XREF: sub_71B4C Moved
		move.b	2(a6),1(a6)
		lea	$4E(a6),a0
		moveq	#$30,d0
		moveq	#9,d1
...
;		lea	($A04000).l,a0
		...
		move.b	#$2A,($A04000).l
		startZ80
locret_72714:	; Clownacy | Recycling!
		rts
...
;		lea	PSG_Index(pc),a0
		subq.w	#1,d0
		lsl.w	#2,d0			; Clownacy | Replaced with the faster add instruction
;		add.w	d0,d0
;		add.w	d0,d0
		movea.l	(a0,d0.w),a0
PSGDoVolFX_Loop:
		...
		bpl.s	loc_72960		; If it is not a terminator, branch
		cmpi.b	#$80,d0
		beq.s	loc_7299A
...
VolEnv_Jump2Idx:
	move.b	1(a0,d0.w),$C(a5)	; Change flutter index to the byte following the flag
...
; ===========================================================================
;		bra.w	loc_72BEE		; $FF, $1F	Clownacy | Was $ED
; ===========================================================================
...
loc_72B14:				; XREF: loc_72A64
	; Clownacy | Here's my fix to allow cfFadeInToPrevious to be used on FM/PSG tracks
		tst.b	8(a6)	; Clownacy | Is this running on the DAC channel
		bmi.s	dactrack			; If so, branch
...
; S3K's zComputeNoteDuration
	...
	move.b	2(a5),d0	; Get tempo divider for this track
...
@skip:
	move.b	d1,$12(a5)	; Note fill timeout
	move.b	d1,$13(a5)	; Note fill master
	rts		
; ===========================================================================
...
; ===========================================================================
;cfSilenceStopTrack:
;	pea	cfStopTrack(pc)	; Clownacy | Save 2 cycles
...
cfLoopContinuousSFX:
    if EnableContSFX
	btst	#7,$1C(a6)	; Is the flag for continuous playback mode set?
	bne.s	@continuousmode				; If so, branch
	clr.b	$1A(a6)			; Communicate that there is no continuous SFX playing
;	bclr	f_continuous_sfx,misc_flags(a6)		; Clear flag for continuous playback mode (redundant. For this code to be processed, it would have to already be zero)
	addq.w	#2,a4	; Clownacy | Advance readng counter to skip the address
	rts

@continuousmode:
	subq.b	#1,$1B(a6)		; Mark one channel as processed
	bne.w	loc_72E2C				; If that wan't the last channel, branch
	bclr	#7,$1C(a6)	; If it was, clear flag for continuous playback mode...
	bra.w	loc_72E2C				; ...and then branch
    endc
; ===========================================================================		
	include	'MegaPCM.asm'
...
Music05:	include	sound\music05.asm
		even
Music06:	include	sound\sylvcastleact2.asm
		even
...
Music0B:	include sound\BossS1MSv2.asm
		even
Music0C:	include	"sound\wwbgf.asm"
		even
Music0D:
...
MusicE5:	include	"sound\shzact2.asm"
		even
MusicE6:	include	"sound\csact2v6.asm"
		even
...
MusicFA:	include	sound\musicFA.asm
		even
MusicFB:	include	sound\SylvaniaCastleact3v4.asm
		even
...
SoundDB:	incbin	sound\soundDB.bin
		even
SoundDC:	include	"sound\Selectv4.asm"
		even
