...
v_lvllayout:	= $FFFFA400		; layout is read from rom now, this contains pointers to the location of level layouts on ROM now 			; level and background layouts ($400 bytes)
v_screenposy_last   = $FFFFFE2A   	; S3K Object manager
v_screenposx_coarse = $FFFFFC14   	;         ''
v_screenposy_coarse = $FFFFFC18   	;         ''
v_objstate:	= $FFFF8000		; object state list ($300 bytes)

v_variables        = v_objstate+$300
...
v_colladdr1:	equ $FFFFF102	; (4 bytes)
v_colladdr2:	equ $FFFFF106	; (4 bytes)
