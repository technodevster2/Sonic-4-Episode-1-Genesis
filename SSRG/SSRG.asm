...
SRG_DumpPal:
		...
		lea	($FFFFD000).w,a1			; load object ram to a1
		move.b	#$01,(a1)				; set "S"
...
SSRGScreen_Finish:
		move.b	#$34,($FFFFF600).w ; go to next screen
		rts						; return
...
SRG_ScrollFG:
		lea	($FFFFCDCC).w,a1			; load the horizontal scroll buffer address
		moveq	#$00,d0					; clear d0
...
OSN_MoveIn:
		...
		move.w	#$3C,d0				; set to play spinning SFX
...
OS_PlaySound:
		addq.b	#$02,$24(a0)				; increase routine counter
		moveq	#$FFFFFFB6,d0				; set to play spin release SFX
		jsr	PlaySound_Special			; play SFX
...
OS_SpinIn:
		...
		addq.b	#$02,$24(a0)				; increase routine counter
		moveq	#$FFFFFF96,d0				; set to play spiked chandelier SFX
		jsr	PlaySound_Special			; play SFX
